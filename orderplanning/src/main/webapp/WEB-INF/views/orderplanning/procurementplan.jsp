...
<body>
						...
						<tbody>
							...
							<c:forEach var="list" items="${ ppList }">
								...
								<tr>
									...
									<td style="text-align: center;">
										<c:if test="${ list.procurement_date == null }">
											<input type="date" id="date" name="procurementPlanVOList[${ no - 1 }].procurement_date" max='<fmt:formatDate value="${ list.production_date }" pattern="yyyy-MM-dd"/>'>
										</c:if>
										<c:if test="${ list.procurement_date != null }">
	...
	<script>
		if (document.getElementById("flag").value == 2) {
			...
			document.getElementById("procurementplan").style.color = "#000000";
			document.getElementById("procurementplan").style.fontWeight = "bold";
