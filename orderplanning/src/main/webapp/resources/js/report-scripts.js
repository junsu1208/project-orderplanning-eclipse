...
function fetchTransactionsListReport() {
    ...
    document.getElementById('currentReport.exportTo').value = txnExportValue1.options[txnExportValue1.selectedIndex].text; //이 함수랑 연결된 버튼은 그냥 "버튼"이었는데 이 함수로 폼을 만들어서 transactionsListReport 여기로 요청을 보내는 거 같음. 서버에서 content type 결정해서 뿌려주는 것 같음. 서버말고 화면에서 만들기를 원하면 새로 작성해야함.
    document.forms[0].target = 'reportContent';
